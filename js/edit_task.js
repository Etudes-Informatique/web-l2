                ...
                success: function(task) {
                    ...
                    editorDiv.empty().append(`
                        <form class="form-update-task" data-id="${taskId}" style="margin-top:10px; border-top:1px solid #ccc; padding-top:10px;">
                            <input type="text" name="title" value="${task.title}" required style="width:94%;">
                            <textarea name="description" required style="width:94%;">${task.description}</textarea>
                            
                            <label>Priorité :</label>
                            <select name="priority" style="width:98%;">
                            ...
                            <select name="category_id" class="task-category-select" style="width:98%;">
                            ...
                            <select name="status" style="width:98%;">
                                ...
                                <option value="Terminé" ${task.status === 'Fini' ? 'selected' : ''}>Terminé</option>
                            </select>

                            <label>Échéance :</label><br>
                            <input type="date" name="deadline" value="${task.deadline ? task.deadline.replace(' ', 'T').substring(0, 16) : ''}" style="width:95%;">
                            
                            <button id="edit-submit" type="submit">Enregistrer</button>
                            <button id="edit-cancel" type="button" onclick="location.reload()">Annuler</button>
                        </form>
                    `);
